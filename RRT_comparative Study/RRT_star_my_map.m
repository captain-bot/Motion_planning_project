...
%         end
        ...
        % Get the node indices within K radius from q_new
        near_idx = [];
        for ii = 1:length(TREE)-1
            cn = TREE(ii);
            dist_cn_qnew = norm(cn.coord - q_new.coord);
            if dist_cn_qnew < k_rad
                near_idx = [near_idx, ii];
            end            
        end

        % Select best parent
        C_min = q_new.cost;
        q_min_idx = q_new.parent;
        for jj = near_idx
            alternative_cost = TREE(jj).cost + norm(TREE(jj).coord - q_new.coord);
            if alternative_cost < C_min
                C_min = alternative_cost;
                q_min_idx = jj;
            end
        end
        q_new.parent = q_min_idx;
        q_new.cost = C_min;
        
        % Perform Rewiring
        for kk = near_idx
            new_cost = q_new.cost + norm(q_new.coord - TREE(kk).coord);
            if new_cost < TREE(kk).cost
                TREE(kk).parent = node_count + 1;
            end
        end
...
xlabel('x [m]')
ylabel('y [m]')
title('RRT*-2D Implemention r_k=0.2')
grid on
% axis('equal')
