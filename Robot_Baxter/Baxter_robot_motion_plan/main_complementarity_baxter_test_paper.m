...
global safe_dist;       global type_joint;      global num_dof;    global num_contacts;    
global array_vc;        global array_vin;       global array_qo;
...
%              Plot the Motion         //
%////////////////////////////////////////
stack_qo = []; stack_ip_vel = []; stack_vc= [];
for i = 1:length(path_node_ind)-1
    ...
    stack_qo = [stack_qo, array_qo];
    stack_ip_vel = [stack_ip_vel, array_vin];
    stack_vc = [stack_vc, array_vc];
end

tm = 0:dh:(length(stack_qo)-1)*dh;
for ii = 1:size(stack_qo, 1)
