...
for i = 1:length(path_node_ind)-1
    if i < length(path_node_ind)
        ...
        bax_local_planner(tree.nodes(path_node_ind(i),1:num_dof),goal_st(1:num_dof),tree.nodes(path_node_ind(i),num_dof+1:2*num_dof),dt,dh,tol);
    end
%     node2exp = tree.nodes(path_node_ind(i),:);
    for j = 1:size(array_qo,2)    
        m(kk) = visualizeBaxter(array_qo(:,j));
        kk = kk + 1;
    end
