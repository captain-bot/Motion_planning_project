...
goal_st = [0.1, -0.1, 0.1, 0.4, 0.3, 0.3, 0.1, zeros(1,7)];
...
plot_initial_goal(initial_st, 'r');  % Plot start state
hold on
plot_initial_goal(goal_st, 'g');     % Plot goal state
...
for i = 1:length(path_node_ind)-1
    ...
    for j = 1:size(array_qo,2)/2    
        visualizeBaxter(array_qo(:,2*j));
    end
end
...
%//////////////////////////////////////////////////
function [] = plot_initial_goal(theta, colr)
    ...
    % Plot
    plot3(g_end(1,4),g_end(2,4),g_end(3,4),'ko','MarkerSize',12,'MarkerfaceColor',colr);
end

%//////////////////////////////////////////////////
%               Visualize the motion             //
%//////////////////////////////////////////////////
function [mov] = visualizeBaxter(theta)
    global g_st0; global type_joint; global wr; global qr; global base_mat;
    ...
    % Plot robot
    mov = draw_baxter(base_mat, transform_upto_joint, g_end);
end